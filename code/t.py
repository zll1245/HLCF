...
class DeepCrackTrainer(nn.Module):
    ...
    def val_op(self, input, target):
        ...
        p_fuse4 = torch.sigmoid(pred_fuse5[0].cpu().squeeze())
        # print(p_fuse1.shape)
        save_pred_fuse4 = torch.zeros((512, 512))
        save_pred_fuse4[:512, :] = p_fuse4
        # save_name = os.path.join(save_path, os.path.split(names[1])[1])
        save_pred_fuse4 = save_pred_fuse4.numpy() * 255
        cv2.imwrite("deepcrack_results/fuse5.png", save_pred_fuse4.astype(np.uint8))
