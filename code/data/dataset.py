...
import os
#
# def readIndex(index_img_path, index_lab_path,shuffle=False):
#     img_list = []
...
def readIndex(img_dir, lab_dir, shuffle=False):
    img_list = []
    for img_file in os.listdir(img_dir):
        if img_file.endswith(('.png', '.jpg', '.jpeg', '.bmp', '.tiff')):
            img_path = os.path.join(img_dir, img_file)
            label_path = os.path.join(lab_dir, img_file)
            if os.path.exists(label_path):
                img_list.append((img_path, label_path))
            else:
                print(f"Label file {label_path} not found for image {img_path}")
    if shuffle:
        random.shuffle(img_list)
